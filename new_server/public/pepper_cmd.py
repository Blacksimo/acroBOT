...
class PepperRobot:
    ...
    def showurl(self, weburl):
        #ex: http://198.18.0.1/apps/spqrel/ + weburl
        strurl = weburl
        print "URL: ",strurl
        self.tablet_service.showWebview(strurl)

    def openWebService(self, url):
        #print(self.tablet_service.getWifiStatus())
        self.tablet_service.disconnectWifi()
        self.tablet_service.configureWifi('wpa', 'SPQREL', 'robocup2017')
        self.tablet_service.connectWifi('SPQREL')
        self.tablet_service.showurl(url)
